...
from rag_integration_node import RAGIntegrationNode, rag_retrieval_node
...
async def initialize_rag():
    """Initialize RAG node at startup"""
    global rag_node
    try:
        rag_node = RAGIntegrationNode(chroma_db_path="./medical_data_store")
        print(f"✅ RAG Integration Node initialized")
        print(f"📊 Vector Store Contains: {rag_node.rag_crawler.get_collection_stats()}")
        return True
    except Exception as e:
        print(f"⚠️  Warning: RAG initialization failed: {e}")
        print(f"   Continuing without RAG - ensure medical_data_store exists")
        return False
...
async def lifespan(app: FastAPI):
    ...
    if not await initialize_chatbot():
        print(
            "⚠️  Warning: Failed to initialize chatbot. API will still work but chatbot endpoints may fail."
        )
    ...
    print("\n🛑 Shutting down Medi-Mind Backend...\n")


# Initialize FastAPI app
app = FastAPI(
    title="Medi-Mind Backend with RAG",
    description="FastAPI backend for the Medi-Mind medical assistant application with Retrieval Augmented Generation",
    version="0.2.0",
    lifespan=lifespan,
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "Medi-Mind Backend API with RAG",
        "status": "running",
        "rag_enabled": rag_node is not None
    }
...
async def health_check():
    ...
    stats = {}
    if rag_node:
        stats = rag_node.rag_crawler.get_collection_stats()
    
    return {
        "status": "healthy",
        "chatbot_initialized": chatbot_graph is not None,
        "rag_initialized": rag_node is not None,
...
async def chat_simple(request: SimpleChatRequest):
    """
    Simple chat endpoint that takes a message with RAG integration.
    Conversation history is maintained on the backend per session_id.
    Uses cosine similarity to retrieve medical context before LLM inference.
    """
    ...
    try:
        ...
        # RAG Integration: Augment state with medical context if enabled
        rag_context_used = False
        sources = []
        
        if request.use_rag and rag_node is not None:
            try:
                print(f"🔍 Retrieving medical context using cosine similarity...")
                state = await rag_retrieval_node(state, rag_node)
...
async def get_rag_stats():
    ...
    return {
        "status": "ok",
        "vector_store": stats,
        "cache_size": len(rag_node.retrieval_cache)
    }


@app.post("/rag/query")
async def query_rag(query: str, n_results: int = 5):
    """Query RAG system directly (for testing)"""
    if rag_node is None:
        raise HTTPException(status_code=503, detail="RAG system not initialized")
    
    try:
        results = rag_node.retrieve_medical_context(query, n_results)
        return {
            "query": query,
